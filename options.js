...
var assistText = chrome.i18n.getMessage("of_follow_feed_url_assist");
...
function main() {
  document.title = chrome.i18n.getMessage("of_follow_edit_dialog_title");
  ...
  if (!storageEnabled) {
    ...
    alert(chrome.i18n.getMessage("of_follow_no_localstorage"));
    return;
  }
  ...
  for (i = 0; i < feedReaderList.length; ++i) {
    ...
    if (isDefaultReader(feedReaderList[i].url))
      description += " " + chrome.i18n.getMessage("of_follow_default");
...
function resetList() {
  if (!confirm(chrome.i18n.getMessage("of_follow_reset_list_confirm"))) {
    return;
  }
  ...
  if (index == -1) {
    ...
    document.getElementById('descriptionText').value =
        oldOption.text.replace(
            ' ' + chrome.i18n.getMessage("of_follow_default"), '');
...
function removeReader() {
  ...
  var reply =
    confirm(chrome.i18n.getMessage("of_follow_remove_confirm",
...
document.addEventListener('DOMContentLoaded', function () {
  // Localize.
  i18nReplace('of_follow_options');
  i18nReplaceImpl('addReader', 'of_follow_add_reader');
  i18nReplaceImpl('editReader', 'of_follow_edit_reader');
  i18nReplaceImpl('removeReader', 'of_follow_remove_reader');
  i18nReplaceImpl('setDefault', 'of_follow_make_default_reader');
  i18nReplaceImpl('resetList', 'of_follow_reset_list');
  i18nReplace('of_follow_always_use_default');
  i18nReplaceImpl('dialogHeader', 'of_follow_edit_dialog_title');
  i18nReplace('of_follow_feed_description');
  i18nReplace('of_follow_feed_url');
  i18nReplaceImpl('save', 'of_follow_save_button', 'value');
  i18nReplaceImpl('of_follow_close_button',
                  'of_follow_close_button', 'value');
  ...
  listen('of_follow_close_button', 'click', hideDialog);
