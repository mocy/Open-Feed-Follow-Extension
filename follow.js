...
var unknownName = chrome.i18n.getMessage("of_follow_unknown_feed_name");
...
function main() {
  ...
  if (storageEnabled) {
    ...
    readerDropdown.options[i] =
        new Option(chrome.i18n.getMessage("of_follow_manage_label"), "");
...
function setFeedTitle(title) {
  ...
  titleTag.textContent =
      chrome.i18n.getMessage("of_follow_feed_for", title);
}

// Handles errors during the XMLHttpRequest.
function handleError() {
  handleFeedParsingFailed(
      chrome.i18n.getMessage("of_follow_error_fetching"));
}
...
function handleResponse() {
  ...
  if (!doc) {
    ...
    if (!doc) {
      handleFeedParsingFailed(
          chrome.i18n.getMessage("of_follow_not_valid_feed"));
      return;
    }
  }
  ...
  if (entries.length == 0) {
    handleFeedParsingFailed(
        chrome.i18n.getMessage("of_follow_no_entries"))
    return;
  }
...
document.addEventListener('DOMContentLoaded', function () {
  document.title =
      chrome.i18n.getMessage("of_follow_default_title");
  i18nReplace('of_follow_follow_using');
  i18nReplace('of_follow_follow_button');
  i18nReplace('of_follow_always_use');
  i18nReplace('of_follow_feed_preview');
  i18nReplaceImpl('feedUrl', 'of_follow_feed_link', '');

  var dropdown = document.getElementById('readerDropdown');
  dropdown.addEventListener('change', onSelectChanged);
  var button = document.getElementById('of_follow_follow_button');
