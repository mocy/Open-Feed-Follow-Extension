...
function main() {
  chrome.tabs.getSelected(null, function(tab) {
    ...
    if (feeds.length == 1) {
      // Only one feed, no need for a bubble; go straight to the follow page.
      preview(feeds[0].href);
    } else {
      var content = document.getElementById('content');
      var heading = document.getElementById('heading');
      heading.innerText =
          chrome.i18n.getMessage("of_follow_action_title");
...
function preview(feed_url) {
  // See if we need to skip the preview page and follow directly.
  var url = "";
  if (window.localStorage && window.localStorage.showPreviewPage == "No") {
    ...
    url = "follow.html?" + encodeURIComponent(feed_url);
  }
